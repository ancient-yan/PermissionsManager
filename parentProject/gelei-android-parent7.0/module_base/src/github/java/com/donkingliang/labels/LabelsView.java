...
public class LabelsView extends ViewGroup implements View.OnClickListener {
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            int type = mTypedArray.getInt(R.styleable.LabelsView_lv_selectType, 1);
            mSelectType = SelectType.get(type);

            mMaxSelect = mTypedArray.getInteger(R.styleable.LabelsView_lv_maxSelect, 0);
            mMinSelect = mTypedArray.getInteger(R.styleable.LabelsView_lv_minSelect, 0);
            mMaxLines = mTypedArray.getInteger(R.styleable.LabelsView_lv_maxLines, 0);
            isIndicator = mTypedArray.getBoolean(R.styleable.LabelsView_lv_isIndicator, false);
            mTextColor = mTypedArray.getColorStateList(R.styleable.LabelsView_lv_labelTextColor);
            mTextSize = mTypedArray.getDimension(R.styleable.LabelsView_lv_labelTextSize,
                    sp2px(context, 14));
            mTextPaddingLeft = mTypedArray.getDimensionPixelOffset(
                    R.styleable.LabelsView_lv_labelTextPaddingLeft, 0);
            mTextPaddingTop = mTypedArray.getDimensionPixelOffset(
                    R.styleable.LabelsView_lv_labelTextPaddingTop, 0);
            mTextPaddingRight = mTypedArray.getDimensionPixelOffset(
                    R.styleable.LabelsView_lv_labelTextPaddingRight, 0);
            mTextPaddingBottom = mTypedArray.getDimensionPixelOffset(
                    R.styleable.LabelsView_lv_labelTextPaddingBottom, 0);
            mLineMargin = mTypedArray.getDimensionPixelOffset(R.styleable.LabelsView_lv_lineMargin, 0);
            mWordMargin = mTypedArray.getDimensionPixelOffset(R.styleable.LabelsView_lv_wordMargin, 0);
            int labelBgResId = mTypedArray.getResourceId(R.styleable.LabelsView_lv_labelBackground, 0);
            if (labelBgResId != 0) {
                mLabelBg = getResources().getDrawable(labelBgResId);
            } else {
                int labelBgColor = mTypedArray.getColor(R.styleable.LabelsView_lv_labelBackground, Color.TRANSPARENT);
...
//        label.setEllipsize(TextUtils.TruncateAt.END);
                ...
                if (mSelectType != SelectType.NONE) {
                    if (label.isSelected()) {
                        ...
                        if (mSelectType == SelectType.SINGLE || mSelectType == SelectType.SINGLE_IRREVOCABLY) {
                            ...
                            innerClearAllSelectExceptPosition(0);//最多只会选两个，根据具体业务更改的默认第一个必选
                            setLabelSelect(label, true);
                        }
                    }
                }
     ...
     * @param position
     */
    private void innerClearAllSelectExceptPosition(int position) {
        innerClearAllSelect();
        setLabelSelect((TextView) getChildAt(position), true);
    }
